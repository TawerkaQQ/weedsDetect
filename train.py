...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--optimizer', type=str, 
                        choices=['SGD', 'AdamW', 'NAdam', 'RMSprop', 'Adam'],
                        default='Adam')
...
def get_optim(model, optim_name: str, lr: float):
    optims = {
        'SGD': torch.optim.SGD, 
    ...
    optimizer = optims[optim_name](model.parameters(), lr=lr)
    return optimizer 
...
def main() -> None:
    ...
    test_dataset = datasets.ImageFolder(args.test_dataset, train_transforms) # СДЕЛАЛ ДЛЯ ДАЛЬНЕЙШЕГО ВЫЗОВА СКРИПТА С ПРОВЕРКОЙ
    ...
    # также подгрузчик для дальнейшего вызова скрипта тестирования
    ...
    optimizer = get_optim(model,args.optimizer, lr=args.lr) #ASFNASLKFSAJFKLASFJA:SKLFJA:LSFJAS:LFJKAS:LFASL:FJAS:LFJAS:LF
    print(optimizer)

    # optimizer = torch.optim.Adam(model.parameters(), lr=args.lr) # TRUE 
